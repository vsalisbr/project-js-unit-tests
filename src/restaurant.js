...
const createMenu = (obj) => ({
  ...
  consumption: [],
  order(item) {
    const foods = Object.keys(this.fetchMenu().food);
    const drinks = Object.keys(this.fetchMenu().drink);
    if (foods.indexOf(item) !== -1 || drinks.indexOf(item) !== -1) {
  ...
  pay() {
    ...
    for (let i = 0; i < this.consumption.length; i += 1) {
      if (menu.food[this.consumption[i]]) {
        price += menu.food[this.consumption[i]];
      } else {
        price += menu.drink[this.consumption[i]];
      }
    }
    return price;
  },
